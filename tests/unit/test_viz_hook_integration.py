...
def _write_synthetic_source(tmp_path: Path, n_bars: int = 200, seed: int = 42) -> Path:
    df = make_synthetic_ohlcv(n_bars=n_bars, seed=seed)
...
# ─────────────────────────────────────────────────────────
def test_recorder_captures_during_real_backtest(tmp_path: Path) -> None:
    src = _write_synthetic_source(tmp_path, n_bars=300, seed=42)
...
# ─────────────────────────────────────────────────────────
def test_viz_does_not_change_final_equity(tmp_path: Path) -> None:
    """V2: chart_hook on/off must produce bit-exact same final_equity."""
    src = _write_synthetic_source(tmp_path, n_bars=200, seed=7)
...
def test_viz_does_not_change_fills(tmp_path: Path) -> None:
    """V2: fill prices/qtys must be bit-exact regardless of chart_hook."""
    src = _write_synthetic_source(tmp_path, n_bars=300, seed=11)
...
# ─────────────────────────────────────────────────────────
def test_api_comment_propagates_to_recorder(tmp_path: Path) -> None:
    ...
    src = _write_synthetic_source(tmp_path, n_bars=20, seed=0)
...
def test_api_comment_includes_bar_index(tmp_path: Path) -> None:
    ...
    src = _write_synthetic_source(tmp_path, n_bars=10, seed=0)
...
# ─────────────────────────────────────────────────────────
def test_api_comment_noop_when_no_chart_hook(tmp_path: Path) -> None:
...
# ─────────────────────────────────────────────────────────
def test_recorder_fill_count_matches_result(tmp_path: Path) -> None:
    src = _write_synthetic_source(tmp_path, n_bars=200, seed=99)
    strategy = PROJECT_ROOT / "strategies" / "rsi_mean_reversion.py"
